...
generate_sources () {
    ...
    mkdir "$sources/$main_framework_name"
    cp -f "Firebase.h" "$sources/$main_framework_name"
    cp -f "module.modulemap" "$sources/$main_framework_name"
    touch "$sources/$main_framework_name/dummy.m" # SPM requires at least one source file
...
if [[ $latest != $current || $debug ]]; then
    ...
    (
        ...
        gh release download --pattern "$main_framework_name.zip" --repo $main_framework_repo
        echo "Unzipping.."
        unzip -q "$main_framework_name.zip"
        echo "Preparing xcframeworks for distribution..."
        cd $main_main_framework_name
        rename_frameworks "_"
        zip_frameworks
